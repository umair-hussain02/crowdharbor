...
export default function Page() {
    ...
    (async () => {
      ...
      try {
        ...
        if (!response.ok || !result?.article) {
          setLoadError(result?.error || 'Could not load this article.');
          return;
        }
  ...
  const save = async (publish = false) => {
    ...
    try {
      ...
      setForm(f => ({ ...f, slug: result.article.slug, status: result.article.status, publishDate: result.article.publishedAt }));
      setSaveStatus('saved');
  ...
  if (loading) {
    return (
      <AdminLayout title="Edit Article" subtitle="Loading…">
        <div style={{ background: '#FFF', borderRadius: '18px', border: '1px solid #E0E0E0', padding: '60px 24px', textAlign: 'center' }}>
          <div style={{ fontSize: '15px', fontWeight: 700 }}>Loading article…</div>
        </div>
      </AdminLayout>
    );
  }
  ...
  if (loadError) {
    return (
      <AdminLayout title="Edit Article" subtitle="">
        <div style={{ background: '#FFF', borderRadius: '18px', border: '1px solid #E0E0E0', padding: '60px 24px', textAlign: 'center' }}>
          <div style={{ fontSize: '15px', fontWeight: 700, marginBottom: '10px', color: '#DC2626' }}>{loadError}</div>
          <Link href="/admin/blog" style={{ fontSize: '13px', color: '#FD6628', fontWeight: 700, textDecoration: 'none' }}>
            ← Back to Blog
          </Link>
        </div>
      </AdminLayout>
